...
// src/app/dashboard/owner/my-project-proposals/accepted/page.tsx
"use client";
...
export default function AcceptedProjectProposalsPage() {
  ...
  // Função para buscar as propostas aceitas
  const fetchProposals = async () => {
    setLoading(true);
    setError(null);
    try {
      const data = await projectProposalService.getMyProjectProposalsAccepted(); // Chama a função para propostas aceitas
      setProposals(data);
    } catch (err: any) {
      setError(err.message || 'Falha ao carregar propostas aceitas.');
      toast.error(err.message || 'Falha ao carregar propostas aceitas.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      ...
      <main className="container mx-auto px-4 mt-8">
        <h1 className="text-3xl font-bold mb-6 text-gray-800">Minhas Propostas Aceitas</h1>
        ...
        {loading ? (
          ...
          <ProjectProposalsTable
            ...
            showActionsForPending={false} // Desabilita os botões de ação para propostas aceitas
          />
        )}
